...
- **Multi-tenant data boundary (vector layer)**: opt-in tenant isolation enforced in the vector store. `VectorStore` / `AsyncVectorStore` reads (`search`, `scroll`, `iter_ids`, `count`) take a `tenant=` that becomes a **mandatory** Qdrant filter AND-combined with any caller `filters` — a `tenant`-scoped read can never see another tenant's points. Writes (`upsert`, `upsert_batch`) take `tenant=` and stamp the `tenant_id` payload key, **overriding** any caller-supplied `tenant_id` so a write can't be redirected into another tenant's namespace; `invalidate` and `set_payload` gain a tenant owner-guard (a caller who knows another tenant's deterministic point id still can't relabel, overwrite, or invalidate it). Chunk ids are **tenant-scoped** (`stable_chunk_id(..., tenant=)`), so two tenants ingesting identical `(source, offset, text)` get distinct ids instead of one clobbering the other; `prune_stale_chunks(..., tenant=)` is likewise scoped so one tenant's re-index can't delete another's same-`source` chunks. `tenant_id` is a protected payload key — only the `Ingestor`'s `tenant` sets it, never metadata or an enrich hook. `Ingestor(..., tenant=...)` stamps every ingested chunk. Migration: `mnemostack tenant-migrate --tenant <id>` stamps existing points (only those lacking a `tenant_id` by default, so it's idempotent; `--all` to force; `--dry-run` to preview) — a server-side payload-only merge, vectors untouched. `mnemostack.vector.TENANT_ID_KEY` is exported. With no `tenant` this is a no-op — single-tenant deployments are unchanged. This is the data-layer foundation; wiring `tenant` through the recall path and authenticated tenant resolution (service keys/scopes) are separate follow-ups.
