...
- **Operator endpoints are unauthenticated.** `/health`, `/healthz`, `/readyz`, `/status`, `/metrics` (and the `inspect` browse views without `--auth`) are not behind the service-key check — they expose version, dependency reachability, and counters. Bind the server to a trusted interface or front it with an authenticating proxy if those are sensitive.
...
- **Migration leaves no stray state, but check pre-existing payloads.** `tenant-migrate` stamps the server-owned `tenant_id`; if a collection previously used `tenant_id` as an *ordinary* payload field, the server-owned value wins on every scoped read/write, but audit such payloads before adopting tenancy.
