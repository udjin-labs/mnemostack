...
def test_a_cache_hit_hands_out_copies_not_the_entry_itself():
    """A cache entry lives for `cache_ttl` and is handed to every request
    repeating this (query, id-set), so returning it by reference makes one
    request's results another's. Consumers DO write to results — the vector
    floor stamps an appended candidate's score, a weak-recall retry writes
    fused RRF scores — and those writes would land in the entry, so every
    later request in the TTL window would read another request's numbers,
    and two concurrent requests would edit one object at once.
    """
    reranker = Reranker(FakeLLM(response="1"))
    stored = [RecallResult(id="A", text="a", score=0.9, payload={"k": "v"}, sources=["vector"])]
    reranker.cache.put("q", [str(r.id) for r in stored], stored)

    handed_out = reranker.rerank(
        "q",
        [RecallResult(id="A", text="a", score=0.9, payload={"k": "v"}, sources=["vector"])],
    )
    assert handed_out[0] is not stored[0]

    handed_out[0].score = 0.0123  # a fused score, as the weak-retry writes
    handed_out[0].sources.append("retry-arm")
    handed_out[0].payload["injected"] = True
