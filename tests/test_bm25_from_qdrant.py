...
def test_bm25_docs_from_qdrant_reads_payload_text_and_metadata():
    client = FakeQdrantClient(
        [
            point(
                "a", {"text": "MERGED pull request 71706", "source_file": "transcript:one.jsonl"}
            ),
...
def test_bm25_retriever_from_qdrant_finds_exact_token():
    client = FakeQdrantClient(
        [
            point(
                "msg-1",
                {"text": "[09:20] user said MERGED! PR 71706", "source_file": "transcript:s.jsonl"},
            ),
...
def test_bm25_from_qdrant_unbounded_by_default():
    client = FakeQdrantClient([point(f"p-{idx}", {"text": f"chunk {idx}"}) for idx in range(5_000)])
