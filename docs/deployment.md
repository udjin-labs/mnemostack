...
- an **HTTP server** for applications and services that call `/recall`, `/answer`, `/memories` (remote write: server-side embedding, tenant-stamped; `DELETE /memories` = hard delete for erasure requests), `/invalidate` (non-destructive retraction), `/triples` (graph write), `/feedback`, `/health` (plus `/healthz` / `/readyz` / `/status` probes), and `/metrics`.
