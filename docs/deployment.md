...
Every authenticated request increments `mnemostack_tenant_requests_total{tenant,endpoint}` (the endpoint label is the route template, e.g. `POST /memories`). The write path additionally attributes embedding cost — `mnemostack_tenant_embedded_chunks_total{tenant}` and `mnemostack_tenant_embedded_chars_total{tenant}` count only items that actually paid a provider call (duplicates cost zero) — and the guardrails count their rejections: `mnemostack_tenant_quota_rejected_total{tenant}` (507s) and `mnemostack_tenant_rate_limited_total{tenant}` (429s). Embedding is paid by the server, so these are the counters that show which tenant is burning the deployment's provider budget. Label cardinality is bounded by the number of issued tenants. With auth off no tenant metrics are emitted (the global counters cover single-tenant deployments).
