...
For multi-tenant deployments, use the built-in **service-key auth** instead of trusting client filters: start the server with `mnemostack serve --auth`, issue per-tenant keys with `mnemostack keys add --tenant <id> --scopes read,write`, and clients present the key via `Authorization: Bearer <key>` or `X-API-Key`. The **tenant is resolved from the key** (a client can't assert another tenant) and enforced through the vector store's tenant filter plus a post-fusion `filter_by_tenant` backstop, so an authenticated caller only ever sees its own tenant's data. `/recall` and `/answer` require `read`; `/feedback`, `/memories`, and `/triples` require `write`; a missing/invalid key is `401`, an insufficient scope `403` (default-deny). Writes are additionally bounded per tenant: the storage quota (`mnemostack quota set <tenant> --max-points N`) is enforced before any `/memories` write (`507 Insufficient Storage` when exceeded — embedding is paid by the SERVER, so cap every external tenant), and the per-tenant rate limit (`--max-rps`) applies to all authenticated calls (`429`). Client metadata cannot set server-owned payload keys (`tenant_id`, underscore-prefixed structural keys, and `tags`/`timestamp` which have dedicated capped fields — rejected with `400`), and duplicate content re-sent to `/memories` is detected against the store and costs zero embedding calls. If you run `mcp-serve` next to `serve`, give it the same quota store (`mcp-serve --quotas-file` / `MNEMOSTACK_QUOTAS_FILE`) so the MCP remember tool enforces the same caps. The operator endpoints stay **unauthenticated even under `--auth`** — `/health`, `/healthz`, `/readyz`, `/status`, `/metrics` expose version, dependency reachability, and counters; protect them at your proxy (or bind to a trusted interface) if that's sensitive. (Auth is off by default — single-tenant deployments are unaffected.) You can still additionally inject a tenant filter at a reverse proxy for defense-in-depth. Upgrade note: on 0.5.0 and earlier, several retrievers ignored `filters=` entirely — see the Security section of the changelog.
