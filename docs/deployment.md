...
- an **HTTP server** for applications and services that call `/recall`, `/answer`, `/memories` (remote write: server-side embedding, tenant-stamped), `/triples` (graph write), `/feedback`, `/health` (plus `/healthz` / `/readyz` / `/status` probes), and `/metrics`.
