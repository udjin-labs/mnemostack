...
Index a corpus **for a tenant** with `mnemostack index --tenant <id>` (the fixed-window indexer, mirroring `index-markdown --tenant`): ids, payload stamps, prune and payload refresh are all scoped, so operator-indexed documents and the same documents written through `/memories` land on the same points instead of two parallel copies. Pre-existing collections created before this release should also run `mnemostack payload-index source --schema keyword` once — the source-scoped lifecycle endpoints and the listing filter on `source`, and an unindexed filter is a collection scan per call (new collections index it automatically).
