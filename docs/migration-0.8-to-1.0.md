...
> pytest -m smoke -q       # end-to-end smoke (in-memory; no services needed)
> ```
...
- **1.0 preserves** the current id derivation — ids computed on 0.8.x remain
  valid.
- **If you must change roots/paths**: re-index the corpus and pass `--prune`
  (full-root walk) to drop the orphaned old chunks. For markdown, `--index-root`
  pins the root explicitly so a nested/single-file refresh keeps the parent
  index's ids.
