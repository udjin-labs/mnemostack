...
return file content. Configure REAL paths: a root whose own path contains a
symlinked component is refused (the root is opened with a component-by-
component no-follow walk, so a directory swapped for a symlink at open time
cannot re-anchor the confinement). The CLI, as the operator surface, trusts the local
...
Notes: `--code` affects which files are walked and how code files are chunked — prose files keep the classic chunker, and all existing machinery (`--prune`, `--refresh-payloads`, provenance `resolve`, tenants, quotas) applies to code chunks unchanged. Window chunking (`--window-size`) is a prose feature and does not apply to code files.
