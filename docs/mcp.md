...
| `as_of` | `string` | `null` | Point-in-time recall (ISO-8601): return facts valid at this world-time instant (`valid_from <= as_of < valid_until`), ignoring later invalidation. Reads `valid_from`/`valid_until` on the stored points; each bound is optional — a missing `valid_from` means indefinite past, a missing `valid_until` indefinite future, and a point carrying neither is valid at every instant, so `as_of` cannot exclude it on world-time grounds. `invalidated_at` is NOT an input: `as_of` ignores it by design, so a point-in-time view can be a SUPERSET of the default one — it returns facts that ordinary recall hides as stale. |
...
changes the order without rewriting it, so the scores need not be in the order
the array is. The number is not a similarity, not a confidence, and not
comparable across calls. See
