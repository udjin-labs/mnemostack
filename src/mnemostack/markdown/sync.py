...
def upsert_markdown_chunks(
    ...
    *,
    ...
    for cid, _text, payload in chunks:
        ...
        # Same parity rule for the write time, and the same shared helper
        # the fixed-window indexer uses: an existing point keeps when it was
        # written, so a warm sync stays a zero-write sync and an old corpus
        # does not have its age reset to today.
        payload = carry_write_time(old, payload)
