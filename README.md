...
Start the server with `--retry-on-weak` if you want a recall that comes back nearly empty to be paraphrased by the answer LLM and asked again, fusing the rounds by reciprocal rank — so a memory that two phrasings both find outranks one that only a single phrasing did, and a later paraphrase can beat an earlier one. What counts as "weak" is a COUNT (`--retry-weak-below`, default 1 — only a recall that returned nothing), not a score: fused scores are RRF values encoding rank, not confidence, so a threshold on them would measure nothing. One extra round, at most two paraphrases, and every retry carries the caller's tenant, filters, validity view and budget unchanged. Two consequences worth knowing before you switch it on: a retried response reports RRF scores (`1/(k+rank)`, on the order of 0.01–0.05) where an unretried one reports the retriever's own similarity scale, so `score` is comparable WITHIN a response and never across a retried and an unretried one — threshold on rank, not on the number; and a retry never returns fewer memories than it was given — if the fused list plus your token budget would hand back less than the recall already had, the original results come back untouched. That is a guarantee about the size of the response, not its membership: at a fixed `limit` a hit that two phrasings found will take the slot of one your first phrasing ranked last, which is what asking again is for. Off by default, and a request can only opt *out* of it — the server pays for the LLM call, so enabling it is the operator's call.
...
It also changes what ranking means, so enable it deliberately: confidence decay reads `last_accessed`, treats a missing value as "no decay", and nothing wrote that key before — so the stage has been inert unless your clients stamped it. Once it is live the effect is asymmetric: a memory recalled **once** and then left cold decays from that moment (≈0.87 after a week, ≈0.56 after a month, floor 0.10 at ~six months, at the preset 30-day half-life), while a memory **never** recalled keeps the undecayed 1.0 forever. That is the Ebbinghaus trade — recency of *use* is the signal — and it multiplies the whole blended score, so `freshness_weight` does not scale it down. Send user actions to `/feedback` to update Q-learning:
