...
The order of `results` is authoritative — do not re-sort by `score`: the pipeline overwrites the fused value and a rerank reorders without rewriting it. See [what `score` is not](docs/api-stability.md#what-score-is-not).
