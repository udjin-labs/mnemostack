...
Start the server with `--retry-on-weak` if you want a recall that comes back nearly empty to be paraphrased by the answer LLM and asked again, merging the second round into the first. What counts as "weak" is a COUNT (`--retry-weak-below`, default 1 — only a recall that returned nothing), not a score: fused scores are RRF values encoding rank, not confidence, so a threshold on them would measure nothing. One extra round, at most two paraphrases, and every retry carries the caller's tenant, filters, validity view and budget unchanged. Off by default, and a request can only opt *out* of it — the server pays for the LLM call, so enabling it is the operator's call.
